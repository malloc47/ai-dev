# Zerobox backend: bwrap+seccomp sandboxing (Linux only).
#
  ...
  mkSandbox =
    ...
    pkgs.writeShellApplication {
      ...
      text = ''
        ...
        # On NixOS, paths like /etc/ssl and $HOME/.config/claude are commonly
        # symlinks into the nix store.  bwrap mounts literal paths, so we must
        # also allow the real targets.
        resolve_symlinks() {
          local path="$1"
